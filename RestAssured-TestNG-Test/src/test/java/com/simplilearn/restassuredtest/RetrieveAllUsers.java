...
public class RetrieveAllUsers {
	...
	public void testGetusers() {
		try {
			...
			RestAssured.given().baseUri(BASE_URL).when().get("/get-users").then().statusCode(200).and()
					.body("message", equalTo("3 Users Fetched Successfully.")).and().body("code", equalTo(101));
			response = RestAssured.given().baseUri(BASE_URL).when().get("/get-users").getBody().asString();
		} catch (Exception e) {
