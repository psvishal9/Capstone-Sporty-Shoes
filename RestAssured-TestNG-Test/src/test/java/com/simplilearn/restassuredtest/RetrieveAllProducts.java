...
public class RetrieveAllProducts {
	...
	public void testGetProduct() {
		try {
			...
			RestAssured.given().baseUri(BASE_URL).when().get("/get-shoes").then().statusCode(200).and().body("code",
					equalTo(101));
			response = RestAssured.given().baseUri(BASE_URL).when().get("/get-shoes").getBody().asString();
		} catch (Exception e) {
