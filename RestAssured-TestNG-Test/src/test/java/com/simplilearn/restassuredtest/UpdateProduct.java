...
public class UpdateProduct {
	...
	public void testGetProduct() {
		try {
			...
			RestAssured.given().baseUri(BASE_URL).queryParam("id", id).queryParam("image", image)
					.queryParam("name", name).queryParam("category", category).queryParam("sizes", sizes)
					.queryParam("price", price).when().put("/update-shoe").then().assertThat().statusCode(200)
					.body("message", equalTo("updated Shoe Updated Successfully."));
			response = RestAssured.given().baseUri(BASE_URL).queryParam("id", id).queryParam("image", image)
					.queryParam("name", name).queryParam("category", category).queryParam("sizes", sizes)
					.queryParam("price", price).when().put("/update-shoe").getBody().asString();
		} catch (Exception e) {
