...
public class AddProduct {
	...
	public void testAddProduct() {
		try {
			...
			int sizes = 9;
			int price = 1000;
			RestAssured.given().baseUri(BASE_URL).queryParam("id", id).queryParam("image", image_url)
					.queryParam("name", name).queryParam("category", category).queryParam("sizes", sizes)
					.queryParam("price", price).when().post("/add-shoe").then().assertThat().statusCode(200)
					.body("message", equalTo("sampleShoe Added Successfully."));
			response = RestAssured.given().baseUri(BASE_URL).queryParam("id", id).queryParam("image", image_url)
					.queryParam("name", name).queryParam("category", category).queryParam("sizes", sizes)
					.queryParam("price", price).when().post("/add-shoe").getBody().asString();
		} catch (Exception e) {
