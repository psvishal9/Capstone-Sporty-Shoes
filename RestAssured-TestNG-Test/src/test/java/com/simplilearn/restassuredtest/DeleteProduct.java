...
public class DeleteProduct {
	...
	public void testDeleteProduct() {
		try {
			...
			int id = 201;
			RestAssured.given().baseUri(BASE_URL).when().delete("/delete-shoe?id=" + id).then().statusCode(200).and()
					.body("message", equalTo("Shoe with ID 201 Deleted Successfully."));
			response = RestAssured.given().baseUri(BASE_URL).when().delete("/delete-shoe?id=" + id).getBody()
					.asString();
		} catch (Exception e) {
