...
public class PostRequestSteps {
	...
	public void hit_the_post_request_to_create_a_new_shoe() {
		...
		int sizes = 9;
		int price = 1000;
		response = request.queryParam("id", id).queryParam("image", image_url).queryParam("name", name)
				.queryParam("category", category).queryParam("sizes", sizes).queryParam("price", price).when()
				.post("/add-shoe");
	}
